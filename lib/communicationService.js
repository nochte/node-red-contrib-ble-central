...
const perform = (peripheral, { operation, service, characteristic, value, meta, withoutResponse }) => {
    ...
    switch (operation) {
      ...
      case 'WRITE':
        ...
        return PeripheralService.fetchServices(peripheral, [service]).then(([s]) => {
          status(peripheral, 'Fetching characteristics on ', service);
          return PeripheralService.fetchCharacteristics(s, [characteristic]).then(([c]) => {
            if(withoutResponse === undefined || withoutResponse === null){ withoutResponse = true; }
            status(peripheral, 'Setting characteristic: ' + characteristic + ' to ' + value + 'withoutResponse=' + withoutResponse);
            return PeripheralService.writeCharacteristic(c, value, withoutResponse);
          });
        }).then((valueBuffer) => {
          return resolve({ operation, service, characteristic, value: valueBuffer, meta });
        }).catch((err) => {
