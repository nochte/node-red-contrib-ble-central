...
const perform = (peripheral, { operation, service, characteristic, value, meta, withResponse }) => {
    ...
    switch (operation) {
      ...
      case 'WRITE':
        ...
        return PeripheralService.fetchServices(peripheral, [service]).then(([s]) => {
          status(peripheral, 'Fetching characteristics on ', service);
          return PeripheralService.fetchCharacteristics(s, [characteristic]).then(([c]) => {
            status(peripheral, 'Setting characteristic: ' + characteristic + ' to ' + value + 'withResponse=' + withResponse);
            return PeripheralService.writeCharacteristic(c, val, withResponse);
          });
        }).then((valueBuffer) => {
          return resolve({ operation, service, characteristic, value: valueBuffer, meta });
        }).catch((err) => {
